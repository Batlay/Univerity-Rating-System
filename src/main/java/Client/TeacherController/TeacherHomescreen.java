...
public class TeacherHomescreen implements Initializable {
    ...
    @FXML
    private TableColumn<Subject, String>  group_column;
    @FXML
    private TableColumn<Subject, Integer>  sem_column;
    @FXML
    ...
    private final  ObservableList<Subject> string_subject = FXCollections.observableArrayList();
    ...
    private void cabinet(ActionEvent event){
        DBUtilis.changeScene(event,"teacher_cabinet.fxml", "Личный кабинет", username);
    }
    ...
    private void subjects_title()  {
        ...
        String selectStmt ="SELECT study_plan.id_group, study_plan.id_semester, study_plan.code_subject, subject.subject_name\n" +
                "                FROM study_plan \n" +
        ...
        connectionTCP.writeUtf(selectStmt);
        connectionTCP.writeObject(Command.TEACHERSUBJECTS);
        connectionTCP.flush();
        List<SubjectO> group  = (List<SubjectO>) connectionTCP.readObject();
        for (SubjectO subjectO : group) {
            Subject e = new Subject(subjectO);
            string_subject.add(e);
        }
        subject_tableview.setItems(string_subject);
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        sem_column.setCellValueFactory(cellData -> cellData.getValue().semesterProperty().asObject());
        group_column.setCellValueFactory(cellData -> cellData.getValue().groupProperty());
